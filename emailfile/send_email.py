...
class SendEmail():
    def __init__(self):
        ...
        self.receivers = [self.email_addressee]

    def send(self):
        # 邮件内容设置
        message = MIMEText('哈哈哈哈哈哈哈', 'plain', 'utf-8')
        # 邮件主题
        message['Subject'] = '您好，测试已完成，测试报告详见正文'
        # 发送方信息
        ...
        try:
            ...
            # 发送
            smtpObj.sendmail(
                self.email_sender, self.receivers, message.as_string())
            # 退出
            smtpObj.quit()

            # 记录日志
            self.log.logInfo("success，邮件发送成功")
